...
export default function OverviewTab({ isMerchant }: { isMerchant: boolean }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    fetchStats();
  }, [user?.id, rewardsRefreshKey]);

  return (
    <>
      {showDeliveryBanner && (
        <div className="bg-[#5E188F] border-b border-white/10 px-4 py-3 flex items-center justify-between gap-3 -mx-4 sm:-mx-6 mb-8">
      ...
      {isMerchant && (
        <Link
          href="/merchant"
          className="mb-6 flex items-center gap-2 text-xs font-bold text-[#F7E7CE]/40 hover:text-[#F7E7CE]/70 uppercase tracking-wide transition-colors"
        >
          <Store className="w-3.5 h-3.5" />
          Switch to your restaurant dashboard
        </Link>
      )}
