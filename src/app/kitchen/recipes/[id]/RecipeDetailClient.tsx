...
// ═══════════════════════════════════════════════════════════════════
export default function RecipeDetailClient({ id, initialRecipe, initialReviews }: RecipeDetailClientProps) {
  ...
  const [recipeLoading, setRecipeLoading] = useState(!initialRecipe);
  const [recipeError, setRecipeError] = useState(false);
  // First effect run reuses the server-fetched recipe/reviews instead of
  // showing a loading spinner over content we already have. Separate refs
  // because the reviews effect re-runs independently once `user` resolves
  // (to fetch personalised "your review" state), while the recipe effect
  // only re-runs when `id` changes.
  const seededRecipeRef = useRef(!!initialRecipe);
  const seededReviewsRef = useRef(!!initialRecipe);
  ...
  // Not `initialReviews.length === 0` — a recipe can genuinely have zero
  // reviews, which must not look like a loading state.
  const [reviewsLoading, setReviewsLoading] = useState(!initialRecipe);
  ...
  const loadRecipe = useCallback(() => {
    const isSeededFirstRun = seededRecipeRef.current;
    seededRecipeRef.current = false;
    if (!isSeededFirstRun) setRecipeLoading(true);
  ...
  const loadReviews = useCallback(async () => {
    const isSeededFirstRun = seededReviewsRef.current;
    seededReviewsRef.current = false;
    if (!isSeededFirstRun) setReviewsLoading(true);
    try {
