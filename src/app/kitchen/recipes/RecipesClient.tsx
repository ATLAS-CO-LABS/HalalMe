      ...
      <Link href={`/kitchen/recipes/${recipe.id}`}>
        ...
        >
          ...
          <div className="p-4">
            ...
            <p className="text-xs mb-3 line-clamp-2 font-normal" style={{ color: `color-mix(in oklab, var(--hm-text) 72%, var(--hm-lm-anchor))` }}>
              {recipe.description}
            </p>

            <div className="flex items-center justify-between mb-2.5">
              <div className="flex items-center gap-1.5 text-xs" style={{ color: `color-mix(in oklab, var(--hm-text) 72%, var(--hm-lm-anchor))` }}>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5 ml-2 min-w-0">
                ...
                <span className="text-xs truncate" style={{ color: `color-mix(in oklab, var(--hm-text) 60%, var(--hm-lm-anchor))` }}>
  ...
  if (tab === "mine") {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-sm mb-6" style={{ color: `color-mix(in oklab, var(--hm-text) 72%, var(--hm-lm-anchor))` }}>
  ...
  if (tab === "saved") {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-sm" style={{ color: `color-mix(in oklab, var(--hm-text) 72%, var(--hm-lm-anchor))` }}>Tap the bookmark icon on any recipe to save it here.</p>
      </div>
    );
  }
  return (
    <div className="text-center py-20">
      ...
      <p className="text-sm" style={{ color: `color-mix(in oklab, var(--hm-text) 72%, var(--hm-lm-anchor))` }}>Try adjusting your search or filters.</p>
    </div>
  );
}
...
function RecipesContent({ initialRecipes, initialHasMore }: RecipesContentProps) {
  const { user } = useAuth();
  ...
  // Browse-all state - initialise from the server-rendered first page (so
  // crawlers and first paint get real recipes, not an empty list), falling
  // back to the in-session cache for instant back-navigation.
  const [allRecipes, setAllRecipes]       = useState<Recipe[]>(() => {
    if (_recipesCache.length === 0 && initialRecipes.length > 0) _recipesCache = initialRecipes;
    return _recipesCache.length > 0 ? _recipesCache : initialRecipes;
  });
  const [allLoading, setAllLoading]       = useState(
    _recipesCache.length === 0 && initialRecipes.length === 0,
  );
  const [page, setPage]                   = useState(1);
  const [hasMore, setHasMore]             = useState(
    _recipesCache.length > 0 ? false : initialHasMore,
  );
  // Skips the very first client-side refetch when the server already
  // supplied the default (unfiltered, page 1) result set — avoids a
  // pointless loading flash immediately after hydration.
  ...
  useEffect(() => {
    const isDefaultFilters = !searchQuery && selectedDifficulty === "All" && selectedCuisine === "All";
    if (!skippedInitialFetch.current) {
      skippedInitialFetch.current = true;
      if (isDefaultFilters && (initialRecipes.length > 0 || _recipesCache.length > 0)) return;
    }
    loadAllRecipes();
        ...
        <div className="mx-auto max-w-7xl px-4 md:px-6 py-4 md:py-5">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
            <div className="flex items-center gap-3 md:gap-4">
              <Link href="/kitchen" aria-label="Back to Kitchen" className="p-2 -m-2">
                <motion.span
              ...
              <div>
                ...
                <p className="mt-0.5 text-sm font-normal" style={{ color: `color-mix(in oklab, var(--hm-text) 72%, var(--hm-lm-anchor))` }}>
          ...
          <div className="flex flex-col md:flex-row gap-2">
            <div className="flex-1 relative">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: `color-mix(in oklab, var(--hm-text) 60%, var(--hm-lm-anchor))` }} />
              <input
                type="text"
                  ...
                  <motion.button
                    ...
                    style={
                      selectedDifficulty === d
                        ...
                        : { backgroundColor: BG2, color: `color-mix(in oklab, var(--hm-text) 72%, var(--hm-lm-anchor))`, border: `1px solid color-mix(in oklab, var(--hm-text) 6%, transparent)` }
                    }
              ...
              {CUISINES.map((c) => (
                <motion.button
                  ...
                  style={
                    selectedCuisine === c
                      ? { backgroundColor: MAGENTA, color: 'white' }
                      : { backgroundColor: BG2, color: `color-mix(in oklab, var(--hm-text) 60%, var(--hm-lm-anchor))`, border: `1px solid color-mix(in oklab, var(--hm-text) 3%, transparent)` }
                  }
                  whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
                >
                  {c}
...
export default function RecipesClient({ initialRecipes, initialHasMore }: RecipesContentProps) {
  return (
    <Suspense>
      <RecipesContent initialRecipes={initialRecipes} initialHasMore={initialHasMore} />
    </Suspense>
  );
}
